...
// Preload all emoji audios on bot start
module.exports.onStart = async () => {
  if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });

  console.log("🔄 Preloading emoji audio files...");
  for (const [emoji, url] of Object.entries(emojiAudioMap)) {
    ...
    if (!fs.existsSync(filePath)) {
      try {
        ...
        console.log(`✅ Cached audio for emoji: ${emoji}`);
      } catch (err) {
        console.error(`❌ Failed to cache emoji ${emoji}:`, err);
      }
    }
  }
  console.log("🎉 All emoji audios are ready!");
};
...
module.exports.handleEvent = async ({ api, event }) => {
  ...
  api.sendMessage({ attachment: fs.createReadStream(filePath) }, threadID, null, messageID);
};
