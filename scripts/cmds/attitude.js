module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "𝐀𝐘𝐀𝐍 𝐀𝐇𝐌𝐄𝐃𝐙",
    description: "Send random attitude video",
...
module.exports.run = async ({ api, event }) => {
    ...
    const fileName = `attitude_${Date.now()}.mp4`;
    const filePath = path.join(__dirname, "cache", fileName);

    const sendVideo = () => {
        api.sendMessage({ body: `「 ${msg} 」`, attachment: fs.createReadStream(filePath) }, event.threadID, () => {
    ...
    request(encodeURI(videoLink))
        ...
        .on("error", err => {
            console.log(err);
            api.sendMessage("❌ ভিডিও ডাউনলোড করতে সমস্যা হয়েছে। আবার চেষ্টা করুন।", event.threadID);
        });
};
